...
async fn trace(data: web::Data<RefCell<List>>, trace: web::Json<Trace>) -> web::Json<Response> {
    let mut mutable_list = data.borrow().pathtraces;
    let tracing = trace.0;
...
fn add_to_data(data: &mut Vec<Pathtrace>, trace_data: Box<Trace>) {
    // TODO: create the pathtrace and append to it
    let mut pathtrace = Pathtrace::new();
    pathtrace.push(*trace_data);

    data.pathtraces.push(pathtrace)
}
...
async fn main() -> std::io::Result<()> {
    HttpServer::new(|| {
        App::new()
            .app_data(web::Data::new(List { pathtraces: vec![] }))
            .service(trace)
    })
    .bind(("127.0.0.1", 3030))?
    .run()
